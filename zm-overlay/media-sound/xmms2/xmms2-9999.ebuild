...
MYREPO="devel" # "devel"  ~ main
EGIT_REPO_URI="git://git.xmms.se/xmms2/xmms2-${MYREPO}.git"

src_compile() {
        epatch "${FILESDIR}"/xmms2-no-ldconfig.patch
        epatch "${FILESDIR}"/xmms2-improve-tags.patch
	...
	if use minimal; then
		...
		for p in alsa ssl:airplay ao ffmpeg:avcodec curl avahi:daap \
			aac:faad flac !nocxx:gme shout:ices jack \
			mad mms modplug mpg123 musepack ofa \
